  ...
  - class: EnvVarRequirement
    envDef:
      - envName: BOWTIE_INDEXES
        envValue: $(inputs.bowtie_db.path)
...
inputs:
  ...
  best:
    default: true # clear clip protocol
    type: boolean?
    inputBinding:
      position: 1
      prefix: --best
  ...
  e:
    type: int?
    default: 35 # clear clip protocol
    inputBinding:
      position: 1
      prefix: -e
    doc: |
  ...
  m:
    default: 1 # clear clip protocol only considers uniquely mapped reads
    type: int?
    inputBinding:
      position: 1
      prefix: -m
